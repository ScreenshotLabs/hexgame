...
function evaluateBoard(board: HexTile[], currentPlayer: Player): number {
  const player0Score = board.reduce((sum, tile) => (tile.type === "0" ? sum + 1 : sum), 0)
  const player1Score = board.reduce((sum, tile) => (tile.type === "1" ? sum + 1 : sum), 0)
  return currentPlayer === "0" ? player0Score - player1Score : player1Score - player0Score
}

function generateMoves(board: HexTile[], currentPlayer: Player): Move[] {
  const currentPlayerTiles = board.filter((tile) => tile.type === currentPlayer)
  ...
  currentPlayerTiles.forEach((tile) => {
    ...
    adjacentTiles.forEach((adjacentTile) => {
      ...
      const foundTile = board.find(
        (boardTile) => boardTile.q === adjacentTile.q && boardTile.r === adjacentTile.r
      )
...
function makeMove(board: HexTile[], move: Move, currentPlayer: Player): HexTile[] {
...
export function calculateBestMove(board: HexTile[], currentPlayer: Player): Move | null {
  function minimax(
    board: HexTile[],
    depth: number,
    alpha: number,
    ...
    if (maximizingPlayer) {
      ...
      for (const move of possibleMoves) {
        ...
        const value = minimax(newBoard, depth - 1, alpha, beta, false, currentPlayer)
      ...
      for (const move of possibleMoves) {
        ...
        const value = minimax(newBoard, depth - 1, alpha, beta, true, currentPlayer)
  ...
  for (const move of possibleMoves) {
    const newBoard = makeMove(board, move, currentPlayer)
    const value = minimax(newBoard, MAX_DEPTH, -Infinity, Infinity, false, currentPlayer)
    if (value > bestValue) {
      bestValue = value
      bestMove = move
    }
