...
function findVictoryPath(
  ...
  const startTiles = board.filter(
    (tile) => tile?.type === playerType && tile.isStart === true
  );
  const endTiles = board.filter(
    (tile) => tile?.type === playerType && tile.isEnd === true
  );
...
function findPath(start: Hex, end: Hex, tiles: HexTile[][], grid: Hex[]): any {
  ...
  while (true) {
    ...
    if (
      !currHex ||
      minDist === Infinity ||
      (currHex.q === end.q && currHex.r === end.r)
    ) {
    ...
    if (currHex !== null) {
      neighbors.forEach((neighbor) => {
        ...
        if (!visited[neighborKey]) {
          const newDist =
            dist[`${currHex!.q},${currHex!.r}`]! +
            hexDistance(currHex as Hex, neighbor)!;
...
export function convertTo2DArray(tiles: HexTile[]): HexTile[][] {
  ...
  const arr: HexTile[][] = Array.from({ length: numRows }, () =>
