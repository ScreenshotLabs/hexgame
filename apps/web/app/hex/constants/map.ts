import { defineHex, Grid, rectangle, line, Direction, Traverser } from "honeycomb-grid"
...
// player A traversers
const playerAStart = line({ start: [1, 0], direction: Direction.E, length: BASE_SIZE })
const playerAEnd = line({ start: [1, BASE_SIZE + 1], direction: Direction.E, length: BASE_SIZE })

// player B traversers
const playerBStart = line({ start: [0, 1], direction: Direction.SE, length: BASE_SIZE })
const playerBEnd = line({ start: [BASE_SIZE + 1, 1], direction: Direction.SE, length: BASE_SIZE })
...
for (let i = 1; i <= BASE_SIZE; i++) {
  const vector = line({ start: [i, 1], direction: Direction.SE, length: BASE_SIZE })
